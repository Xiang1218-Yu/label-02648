...
class SchedulingOptimizer:
    ...
    def generate_schedule_recommendations(self, predictions, weather_forecast=None, region_predictions=None):
        """
        生成调度建议报告
        ...
        if weather_forecast is not None:
            ...
            if weather_forecast in weather_adjustments:
                recommendations["weather_adjustments"].append(weather_adjustments[weather_forecast])

        # 3. 区域优先级分析
        if region_predictions is not None:
            if isinstance(region_predictions, pd.DataFrame):
                region_cols = [
                    col for col in region_predictions.columns if col.startswith("region_") and col.endswith("_orders")
                ]
            ...
            # 按订单量排序
            sorted_regions = sorted(zip(region_cols, region_totals), key=lambda x: x[1], reverse=True)

            recommendations["region_priorities"] = [
                {"region": region, "total_orders": int(orders), "priority": idx + 1}
    ...
    def print_recommendations(self, recommendations):
        ...
        for region_info in recommendations["region_priorities"][:5]:
            print(
                f"  优先级{region_info['priority']}: {region_info['region']} " f"(预计{region_info['total_orders']}单)"
            )
        ...
        print(f"\n【高峰时段分配】")
        peak_allocations = sorted(
            recommendations["hourly_allocation"], key=lambda x: x["predicted_orders"], reverse=True
        )[:5]
        for alloc in peak_allocations:
            print(
                f"  {alloc['hour']}点: 预计{alloc['predicted_orders']}单, "
...
if __name__ == "__main__":
    ...
    recommendations = optimizer.generate_schedule_recommendations(
        predictions, weather_forecast=1, region_predictions=region_predictions
    )
