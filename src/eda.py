...
class EDAAnalyzer:
    ...
    def plot_time_series(self, target_col='orders_total', save_path=None):
        ...
        # 1. 完整时间序列
        axes[0].plot(self.df['datetime'], self.df[target_col], 
        ...
        axes[1].plot(daily_orders.index, daily_orders.values, 
    ...
    def plot_distribution(self, target_col='orders_total', save_path=None):
        ...
        # 1. 直方图
        axes[0, 0].hist(self.df[target_col], bins=50, 
        ...
        # 2. 箱线图
        axes[0, 1].boxplot(self.df[target_col], vert=True, patch_artist=True,
        ...
        # 4. 核密度估计图
        self.df[target_col].plot(kind='kde', ax=axes[1, 1], 
    ...
    def plot_categorical_analysis(self, save_path=None):
        ...
        if 'is_weekend' in self.df.columns:
            ...
            axes[0, 0].bar(weekend_labels, weekend_data['mean'], 
        ...
        if 'weather' in self.df.columns:
            ...
            axes[0, 1].bar(range(len(weather_data)), weather_data.values,
        ...
        if 'weekday' in self.df.columns:
            ...
            axes[1, 0].plot(range(7), weekday_data.values, 
        ...
        if 'is_holiday' in self.df.columns:
            ...
            axes[1, 1].bar(holiday_labels, holiday_data['mean'],
    ...
    def plot_region_analysis(self, save_path=None):
        ...
        region_names = [col.replace('region_', '').replace('_orders', '').upper() 
                       for col in region_cols]
        axes[0, 0].bar(region_names, region_totals.values,
        ...
        # 2. 各区域订单量占比饼图
        axes[0, 1].pie(region_totals.values, labels=region_names, autopct='%1.1f%%',
        ...
        for col, name in zip(region_cols, region_names):
            ...
            axes[1, 0].plot(hourly_avg.index, hourly_avg.values, 
                           marker='o', label=f'区域{name}', linewidth=2)
        ...
        bp = axes[1, 1].boxplot(region_data, labels=region_names, 
                               patch_artist=True)
