...
class ModelEvaluator:
    ...
    def plot_predictions(self, X_test, y_test, sample_size=100, save_path=None):
        ...
        for idx, (name, model) in enumerate(self.models.items()):
            ...
            # 预测vs实际散点图
            axes[0, idx].scatter(
                y_test[:sample_size], y_pred[:sample_size], alpha=0.5, s=30, edgecolors="k", linewidths=0.5
            )
            axes[0, idx].plot(
                [y_test.min(), y_test.max()], [y_test.min(), y_test.max()], "r--", lw=2, label="理想预测线"
            )
            axes[0, idx].set_xlabel("实际订单量", fontsize=12)
            axes[0, idx].set_ylabel("预测订单量", fontsize=12)
            axes[0, idx].set_title(
                f'{name} - 预测 vs 实际\nR²={self.metrics[name]["R2"]:.4f}', fontsize=14, fontweight="bold"
            )
            axes[0, idx].legend()
            ...
            axes[1, idx].scatter(
                y_pred[:sample_size], residuals[:sample_size], alpha=0.5, s=30, edgecolors="k", linewidths=0.5
            )
            ...
            axes[1, idx].set_title(
                f'{name} - 残差分析\nMAE={self.metrics[name]["MAE"]:.2f}', fontsize=14, fontweight="bold"
            )
            axes[1, idx].legend()
    ...
    def plot_feature_importance(self, feature_names, top_n=15, save_path=None):
        ...
        for idx, (name, model) in enumerate(self.models.items()):
            if hasattr(model, "feature_importances_"):
                ...
                axes[idx].set_title(f"{name} - Top {top_n} 特征重要性", fontsize=14, fontweight="bold")
                axes[idx].grid(True, alpha=0.3, axis="x")
            else:
                axes[idx].text(0.5, 0.5, f"{name}\n不支持特征重要性分析", ha="center", va="center", fontsize=14)
    ...
    def plot_time_series_comparison(self, y_test, timestamps=None, save_path=None):
        ...
        # 绘制实际值
        ax.plot(timestamps, y_test, label="实际订单量", linewidth=2, color="black", alpha=0.7)

        # 绘制各模型预测值
        colors = ["red", "blue", "green", "orange", "purple"]
        for idx, (name, y_pred) in enumerate(self.predictions.items()):
            ax.plot(timestamps, y_pred, label=f"{name}预测", linewidth=1.5, alpha=0.7, color=colors[idx % len(colors)])
...
if __name__ == "__main__":
    ...
    feature_cols = [
        ...
        if col not in ["datetime", "orders_total"] and not col.startswith("region_") and not col.startswith("busiest_")
    ]
