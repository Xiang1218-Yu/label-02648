...
class TestDataPreprocessor:
    ...
    def test_create_lag_features(self, sample_data):
        preprocessor = DataPreprocessor()
        lags = [1, 2, 3]
        df_lag = preprocessor.create_lag_features(sample_data, target_col='orders_total', lags=lags)
    ...
    def test_create_rolling_features(self, sample_data):
        preprocessor = DataPreprocessor()
        windows = [3, 6]
        df_roll = preprocessor.create_rolling_features(sample_data, target_col='orders_total', windows=windows)
    ...
    def test_encode_categorical(self):
        df = pd.DataFrame({
            'weather': [0, 1, 2, 0, 1],
            'orders_total': [100, 110, 90, 105, 95]
        })
        preprocessor = DataPreprocessor()
