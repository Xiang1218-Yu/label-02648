...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(
                os.path.join(Config.LOGS_DIR, "project.log"), encoding="utf-8"
            ),
...
def main():
    ...
    df_features = preprocessor.create_all_features(
        df_clean, lags=Config.LAG_FEATURES, windows=Config.ROLLING_WINDOWS
    )
    ...
    predictions_df.to_csv(
        os.path.join(Config.RESULTS_DIR, "predictions.csv"), index=False
    )
