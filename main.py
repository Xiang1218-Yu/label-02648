...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(str(Config.LOGS_DIR / "project.log"), encoding="utf-8"),
...
def main():
    ...
    if not os.path.exists(Config.DATA_PATH):
        ...
        df.to_csv(str(Config.DATA_PATH), index=False)
    ...
    if Config.ENABLE_EDA:
        ...
        eda_analyzer.df = df
        eda_analyzer.generate_full_report(output_dir=str(Config.RESULTS_DIR / "eda"))
        logger.info("EDA分析完成")
    ...
    evaluator.plot_predictions(
        X_test, y_test, sample_size=200, save_path=str(Config.RESULTS_DIR / "predictions_comparison.png")
    )

    evaluator.plot_feature_importance(X.columns, top_n=15, save_path=str(Config.RESULTS_DIR / "feature_importance.png"))

    evaluator.plot_time_series_comparison(
        y_test.values, save_path=str(Config.RESULTS_DIR / "time_series_comparison.png")
    )

    # 导出性能指标
    metrics_df = evaluator.export_metrics_to_excel(str(Config.RESULTS_DIR / "performance_metrics.xlsx"))
    ...
    optimizer = SchedulingOptimizer(
        rider_efficiency=Config.RIDER_EFFICIENCY, max_riders_per_region=Config.MAX_RIDERS_PER_REGION
    )

    recommendations = optimizer.generate_schedule_recommendations(
        predictions=future_predictions, weather_forecast=1, region_predictions=region_predictions  # 假设多云天气
    )

    # 保存调度建议
    optimizer.save_recommendations(recommendations, str(Config.RESULTS_DIR / "schedule_recommendations.json"))
    ...
    predictions_df = pd.DataFrame(
        {"actual_orders": y_test.values, f"{best_model_name}_prediction": best_model.predict(X_test)}
    )
    ...
    predictions_df.to_csv(str(Config.RESULTS_DIR / "predictions.csv"), index=False)
