...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(Config.LOGS_DIR / 'project.log', encoding='utf-8'),
...
def main():
    ...
    if Config.ENABLE_EDA:
        ...
        eda_analyzer.generate_full_report(
            output_dir=Config.RESULTS_DIR / 'eda'
        )
        logger.info("EDA分析完成")
    ...
    evaluator.plot_predictions(
        X_test, y_test,
        sample_size=200,
        save_path=Config.RESULTS_DIR / 'predictions_comparison.png'
    )
    ...
    evaluator.plot_time_series_comparison(
        y_test.values,
        save_path=Config.RESULTS_DIR / 'time_series_comparison.png'
    )

    # 导出性能指标
    metrics_df = evaluator.export_metrics_to_excel(
        Config.RESULTS_DIR / 'performance_metrics.xlsx'
    )
    ...
    optimizer.save_recommendations(
        recommendations,
        Config.RESULTS_DIR / 'schedule_recommendations.json'
    )
    ...
    logger.info(f"预测结果已保存: {Config.RESULTS_DIR / 'predictions.csv'}")
